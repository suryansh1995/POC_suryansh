...
@Provider
@ProducerConsumer
public class GetItemDetailsFilter implements ContainerRequestFilter{
	...
	public void filter(ContainerRequestContext requestContext) throws IOException {
		...
		if (authHeader!=null && authHeader.size()>0) {
			...
			else if (Objects.nonNull(user) && (user.get(0).getRole().equalsIgnoreCase("consumer") || 
					user.get(0).getRole().equalsIgnoreCase("producer"))) {
				return;
			} else {
				Response UnAutorizedResource = Response.status(Response.Status.UNAUTHORIZED)
						.entity("Only Producer & Consumer can Get Item By ID")
						.build();
