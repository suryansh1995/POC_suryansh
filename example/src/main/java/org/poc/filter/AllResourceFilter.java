...
@Provider
public class AllResourceFilter implements ContainerRequestFilter{
		...
		if (authHeader!=null && authHeader.size()>0) {
			...
			if(Objects.isNull(user)) {
				throw new UserNotValid(Response.status(400).entity("User Not Valid").build());
					...
					user.get(0).getRole().equalsIgnoreCase("consumer") || user.get(0).getRole().equalsIgnoreCase("admin"))) {
				return;
			} else {
				Response UnAutorizedResource = Response.status(Response.Status.UNAUTHORIZED)
						.entity("Only Producer, Consumer and Admin can Access the application")
						.build();
